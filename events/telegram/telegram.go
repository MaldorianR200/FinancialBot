...
func (p *Processor) processMessage(ctx context.Context, event events.Event) error {
	meta, err := meta(event)
	if err != nil {
		fmt.Println("meta вернул ошибку")
		return e.Wrap("can't process message", err)
	}

	if err := p.doCmd(ctx, event.Text, meta.ChatID, meta.Username, meta.Datetime); err != nil {
		fmt.Println("doCmd вернул ошибку")
		return e.Wrap("can't process message", err)
	}
